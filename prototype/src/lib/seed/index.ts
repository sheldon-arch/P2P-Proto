...
// v1.1 demos (separate files; not regenerated by reseed)
import v11_multi from "./data/v11_multi_supplier.json";
import v11_edge from "./data/v11_edge_cases.json";
import v12_audit from "./data/v12_audit.json";
...
export function buildSeedSnapshot(): Record<string, Row[]> {
  ...
  const edge = v11_edge as { purchaseOrders: Row[]; returns: Row[] };
  const audit = v12_audit as { purchaseOrders: Row[] };
  ...
  return {
    ...
    // POs / GRNs / invoices / payments: history + live + v1.1 edge demo
    purchaseOrders: [...arr(live_pos), ...edge.purchaseOrders, ...audit.purchaseOrders],
    ...
    returns: [...arr(live_returns), ...edge.returns],
